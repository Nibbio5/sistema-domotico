...
void DomoticsSystem::removeDeviceTimer(std::string device) {
    ...
    if(activeIndex != -1) { // check if the device is in the active devices
        ...
        } else if(manualDevice != nullptr) { //check if the device is active is a ManualDevice

            log.addLog(report::message(currentTime, "Rimosso il timer dal dispositivo " + manualDevice->KName));
            manualDevice->removeStopTime();
            switchedDevices.push_back(manualDevice);

        } else  { //if is not a manual device is a CPDevice
...
void DomoticsSystem::checkSchedule() {
    ...
    do { // iterate over the active devices
        ...
        else nowTime = currentTime; //else set the tempo to the current time
        ...
        for(auto it = active_devices.begin(); it != active_devices.end() && active_devices.size()!=0;) { // iterate over the active devices
            ...
            if(ManualDevice *manualDevice = dynamic_cast<ManualDevice *>(device)) { // check if the device is a ManualDevice
                ...
                if(checkScheduleManualDevice(manualDevice, nowTime, startTime.get(), stopTime.get())) { //verify if the device is turned off
                ...
                if(checkScheduleCpDevice(cpDevice, nowTime, startTime.get())) { //verify if the device is turned off
                    it = active_devices.erase(it);  // remove the device from the active devices
                    continue;
                }
            }
        ...
        else ++i; // increment i if the device is not removed
...
bool DomoticsSystem::checkScheduleCpDevice(CPDevice *cpDevice, const Time nowTime, const Time *startTime) {
    ...
    { // check if the starttime/lastActivationTime is before the nowTime and the device is on
    ...
    if(startTime && !cpDevice->is_on() && *startTime <= nowTime && *startTime + duration >= nowTime) { // check if the device is off and the start time is before the current time and the start time + duration is after the current time
...
void DomoticsSystem::balancePower(const std::string &last, const Time &nowTime) {
    ...
    if(KPowerLimit == 0 && powerLoad < 0) { // check if the power limit is 0 and the power load is negative
    ...
    while(-powerLoad > KPowerLimit) { //while the power load is greater than the power limit 
        log.addLog(report::message(nowTime, "Il sistema è in sovraccarico energetico, gli ultimi dispositivi accesi verranno spenti"));
        ...
        for(int i = 0; i < active_devices.size(); i++) {
            ...
            if(active_devices[i]->get_start_time() != nullptr && *active_devices[i]->get_start_time() >= latestTime && active_devices[i]->KName != last && active_devices[i]->is_on()) {
                if(!active_devices[i]->KIsOnWhiteList) onlyWhiteListDevices = false; // check if the device is not in the white list
                latestTime = *active_devices[i]->get_start_time();
                name = active_devices[i]->KName;
            }
        }

        if(onlyWhiteListDevices) { // check if all the devices are in the white list
            ...
            if(it != active_devices.end()) { //if the last is not in the white list
            ...
            } else { // else turn off the last device
        ...
        } else { // else turn off the the non whitelist device before last
            int activeIndex = getIndex(name, true);
            if(activeIndex != -1) {
                log.addLog(report::message(nowTime, "Il dispositivo " + name + " si é spento"));
                active_devices[activeIndex]->switch_off(currentTime);
...
void DomoticsSystem::setDeviceTime(const std::string &device, const Time &start, const Time &stop) {
    ...
    else if(start == currentTime && stop > currentTime) { // check if the start time is the current time and the stop time is after the current time
    ...
    if(activeIndex == -1) { //if the device is not in the active devices add it
        active_devices.push_back(targetDevice);
    } else { // else remove the device from the switched devices
...
std::ostream &operator<<(std::ostream &out, const DomoticsSystem &sys) {// overload the << operator to output the system status not used in the project
...
void DomoticsSystem::resetTime() {
    for(auto *value : switchedDevices) { // reset all the switched devices timers
    ...
    for(auto *value : all_devices) { //repopulate the active devices with the devices that have a start time or timer
...
void DomoticsSystem::resetTimers() { // reset all the timers of the devices
...
void DomoticsSystem::resetAll() { //reset all the settings in the system
    currentTime = Time(0, 0); 
    for(auto *value : all_devices) {
...
void DomoticsSystem::endDay() { // turn off all the devices
