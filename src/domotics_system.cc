...
*   std::vector<T>   DomoticsSystem::getDevices () { 
    ...
    while (std::getline(file, line)) {
        if (line.find("\"Nome\":") != std::string::npos) {
            ...
            nome = nome.substr(1, nome.size() - 3); // Rimuove virgolette e virgola finale
            ...
            if (durataAccensione == "Manuale") {
                dispositivi.emplace_back(ManualDevice(nome, durataAccensione, produzioneConsumo));
            } else {
                dispositivi.emplace_back(CPDevice(nome, durataAccensione, produzioneConsumo));
            }
        }
    }
